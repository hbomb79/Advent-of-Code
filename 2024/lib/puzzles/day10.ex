defmodule Puzzles.Day10 do
  ...
  def trails_1(grid, start), do: dfs(grid, :queue.from_list([[start]]), MapSet.new(start))
  ...
  def dfs(grid, queue, visited, done \\ []) do
    case :queue.out(queue) do
      {{:value, [hd | _] = q}, queue} ->
        cond do
          Grid.point!(grid, hd) == 9 ->
            dfs(grid, queue, visited, [q | done])
  ...
  def dfs_many(grid, queue, done \\ []) do
    case :queue.out(queue) do
      {{:value, {[hd | _] = q, visited}}, queue} ->
        cond do
          Grid.point!(grid, hd) == 9 ->
            dfs_many(grid, queue, [q | done])
