...
let is_goal target minm (_, (x, y, _, momentum)) =
  Point.equal (x, y) target && momentum >= minm
;;

module Djikstra = Advent.Pathfinding.Djikstra (Node)
...
let _ =
  ...
  let part_one_hl, _ =
    Djikstra.find_shortest_path
      grid
      start_seeds
      ~goal:(is_goal target 0)
      ~adj:(adj_nodes grid 0 3)
      ~g:(g_of_node grid)
  in
  ...
  let part_two_hl, _ =
    Djikstra.find_shortest_path
      grid
      start_seeds
      ~goal:(is_goal target 4)
      ~adj:(adj_nodes grid 4 10)
      ~g:(g_of_node grid)
  in
